...
```{r }
# Filter matrix
movieRealMatrix_dense <- movieRealMatrix[rowCounts(movieRealMatrix) > 20, ]
movieRealMatrix_dense <- movieRealMatrix_dense[, colCounts(movieRealMatrix_dense) > 20]

# Check dimensions
print(dim(movieRealMatrix_dense))

# Recreate cleaned matrix
mat_clean <- as(movieRealMatrix_dense, "matrix")
mat_clean[is.na(mat_clean)] <- 0
mat_clean[is.infinite(mat_clean)] <- 0
movieRealMatrix_dense <- as(mat_clean, "realRatingMatrix")

# Cross-validation and ALS evaluation
m.cross <- evaluationScheme(movieRealMatrix_dense, method = "cross", k = 4, given = 3, goodRating = 5)

m.cross.results.als <- tryCatch({
  evaluate(m.cross, method = "ALS", type = "topNList", n = c(1, 3, 5, 10, 15, 20))
}, error = function(e) {
  message("ALS Evaluation Failed: ", e$message)
  NULL
})
...
```{r }
par(mfrow=c(1,2))
if (!is.null(m.cross.results.als)) {
  par(mfrow = c(1, 2))
  ...
  message("Skipping ALS plots: Evaluation failed or returned NULL.")
}
...
if ("44" %in% rownames(getRatingMatrix(predSVD))) {
  ...
  print("User 44 not found in prediction matrix.")
}
